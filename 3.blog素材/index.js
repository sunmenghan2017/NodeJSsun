...
http.createServer(function(req,res){
    ...
    if(pathName=='/login'){//ok!
        ...
        res.writeHead(200,{'Content-Type':'text/html;charset=utf-8'});//showList(res);
        res.end(listContent)
    }
    else if(pathName=='/listmanager'){//ok!!!
    ...
    else if(pathName.indexOf('detail')>=0){//ok!!!
    ...
    else if(pathName.indexOf('bg')>=0 && req.method == 'GET'){
        var imgSrc = path.join(__dirname,pathName);
        var imgContent = fs.readFileSync(imgSrc);
        res.writeHead(200,"Content-Type","image/jpg");
        res.end(imgContent);
        ...
        var Src = path.join(__dirname,pathName);
        ...
        var Src = path.join(__dirname,pathName);
        var Content = fs.readFileSync(Src);
        res.writeHead(200,"Content-Type","text/js;charset=UFT-8");
        res.end(Content);
    }
    ...
    else if(pathName=='/addlist' && req.method=='POST'){
        uploadFile(req,res);
    }
}).listen(8083);
...
function showList(res){  
    ...
    res.writeHead(200,{"Contsnt-Type":"text/html;text/plain;charset=utf-8"});
    res.write(listStr);
    res.end();
}
...
function uploadFile(req,res){
    ...
    req.on("end",function(){
        if(dataStr){
            ...
            fileObj.chapterName=dataStr;
            fileObj.views=200 ; 
            list.push(fileObj);        
            chapterList.push.apply(chapterList,list);//得到一个数组
            list=[];
        }
   console.log(chapterList);console.log(dataStr);
     var chapterStr = JSON.stringify(chapterList);
     res.writeHead(200,{"Contsnt-Type":"text/html;charset=utf-8"});
     res.end(chapterStr);
    })
}